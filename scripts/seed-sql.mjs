...
export async function buildSeedSql(timestamp = new Date().toISOString()) {
  ...
  return [
    ...
    insert(
      'work_items',
      ['id', 'project_id', 'place_id', 'milestone_id', 'parent_id', 'title', 'detail', 'type', 'priority', 'status', 'assignee', 'estimated_hours', 'required_people', 'suitable_for_dugnad', 'weather_dependency', 'start_at', 'due_date', 'position', 'visibility', 'created_at', 'updated_at'],
      stamped(seedWorkItems).map(item => ({
        ...
        required_people: item.requiredPeople,
        suitable_for_dugnad: item.suitableForDugnad ? 1 : 0,
        weather_dependency: item.weatherDependency,
